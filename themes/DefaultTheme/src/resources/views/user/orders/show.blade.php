        ...
        <div class="row">
                ...
                <div class="col-12">
                    <div class="checkout-alert dt-sn mb-4">
                        ...
                        <div class="checkout-alert-title">
                            <h4> {{ trans('front::messages.profile.your-order-with-order-number') }}<span
            ...
            @elseif(session('transaction-error'))
                <div class="col-12">
                    <div class="checkout-alert dt-sn mb-4">
                        ...
                        <div class="checkout-alert-title">
                            <h4> {{ trans('front::messages.profile.order') }} <span
                                    class="checkout-alert-highlighted checkout-alert-highlighted-success">{{ $order->id }}</span>
                        ...
                        <div class="checkout-alert-content">
                            <p>
                                <span
                                    class="checkout-alert-content-failed">{{ trans('front::messages.profile.system-cancellation') }}</span>
                                <br>
            ...
            <div class="col-12">
                <div class="profile-navbar">
                    <h4>{{ trans('front::messages.profile.order-number') }}<span class="font-en">{{ $order->id }}</span><span>{{ trans('front::messages.profile.recorded-on') }}{{ jdate($order->created_at)->format('%d %B %Y') }}</span>
                    </h4>
                </div>
            </div>
                    ...
                    <div class="row table-draught px-3">
                        ...
                        <div class="col-md-6 col-sm-12">
                            ...
                            @if ($order->shipping_cost)
                                <span
                                    class="value">{{ trans('front::messages.currency.prefix') }}{{ number_format($order->shipping_cost) }}{{ currencyTitle() }}</span>
                        ...
                        @if ($order->hasPhysicalProduct())
                            <div class="col-md-6 col-sm-12">
                                ...
                                <span
                                    class="value">{{ $order->carrier ? $order->carrier->title : trans('front::messages.profile.unknown') }}</span>
                            </div>
                        @endif
                        ...
                        <div class="col-md-6 col-sm-12">
                            ...
                            @else
                                @if(in_array($order->gatewayRelation->key , ['check1' , 'check2' , 'check3' , 'check4' , 'check5' , 'check6']))
                                    <span
                                        class="value">{{ $order->gatewayRelation->name .'( ' .  $order->gatewayRelation->config('percent')  .'% )' }}</span>
                                @else
                        ...
                        <div class="col-md-6 col-sm-12">
                            ...
                            <span class="value">
                                ...
                                @elseif($order->status == 'unpaid')
                                    {{ trans('front::messages.profile.unpaid') }}
                                    ( {{ trans('front::messages.profile.select-payment-gateway') }} )
                                @elseif($order->status == 'pending')
                                    {{ trans('front::messages.profile.pending') }}
                                @else
                                        ...
                                        <div class="col-sm-8 border-none py-0 mb-2 mb-sm-0">
                                            <select class="form-control py-0" name="gateway" required>
                                                ...
                                                @if ($wallet->balance >= $order->price)
                                                    <option
                                                        value="wallet">{{ trans('front::messages.profile.pay-with-wallet') }}</option>
                                                @elseif ($wallet->balance)
                                                    <option
                                                        value="wallet">{{ trans('front::messages.profile.charging-and-payment') }}</option>
                                                @endif
...
{{--                                            <label for="">آپلود چک</label>--}}
{{--                                            <input type="file" required name="fileCheck" class="form-control">--}}
                                        </div>
                        ...
                        <div class="col-md-6 col-sm-12">
                            ...
                            <span
                                class="value">{{ trans('front::messages.currency.prefix') }}{{ number_format($order->totalDiscount()) }}{{ currencyTitle() }}</span>
                        </div>
                        <div class="col-md-6 col-sm-12">
                            <span class="title">{{ trans('front::messages.profile.total-price') }}</span>
                            <span
                                class="value">{{ trans('front::messages.currency.prefix') }}{{ number_format($order->price) }} {{ currencyTitle() }}</span>
                        </div>
                        <div class="col-md-6 col-sm-12">
                            {{--                            <span class="title">{{ trans('front::messages.profile.factor') }}</span>--}}
                            <span class="value">
                                <a target="_blank" href="{{ route('front.orders.print' , [$order->id]) }}">دانلود فاکتور</a></span>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <div class="row mb-3">
                            <div class="col-12">
                                <div class="section-title text-sm-title title-wide no-after-title-wide">
                                    <h2> {{ trans('front::messages.profile.post-status') }} @if($order->shipping_status == 'canceled')
                                            <small
                                                class="text-danger">( {{ trans('front::messages.profile.posting-canceled') }}
                                                )</small>
                                        @endif </h2>
                                </div>
                            </div>
                            ...
                            <div class="col-12">
                                <div class="profile-order-steps  owl-carousel owl-theme">
                                    <div
                                        class="item profile-order-steps-item {{ in_array($order->shipping_status, ['pending', 'wating', 'sent']) ? 'is-active' : '' }}">
                                        <img data-src="{{ theme_asset('img/svg/0eab59b0.svg') }}">
                                        <span>{{ trans('front::messages.profile.pending') }}</span>
                                    </div>

                                    <div
                                        class="item profile-order-steps-item {{ in_array($order->shipping_status, ['wating', 'sent']) ? 'is-active' : '' }}">
                                        <img data-src="{{ theme_asset('img/svg/3db3cdeb.svg') }}">
                                        <span>{{ trans('front::messages.profile.waiting-to-send') }}</span>
                                    </div>
                                    <div
                                        class="item profile-order-steps-item last-item {{ in_array($order->shipping_status, ['sent']) ? 'is-active' : '' }}">
                        ...
                        <table class="table table-order table-order-details">
                            ...
                            @foreach ($order->items as $item)
                                ...
                                <tr>
                                    ...
                                    <td>{{ $item->quantity }}</td>
                                    <td>{{ trans('front::messages.currency.prefix') }}{{ number_format($item->realPrice()) }} {{ currencyTitle() }}</td>
                                    @if(in_array($order->gateway , ['check1' , 'check2' , 'check3' , 'check4' , 'check5' , 'check6']))
                                        <?php
                                            ...
                                            ?>
                                        <td>{{ trans('front::messages.currency.prefix') }}{{  number_format($priceCheck) }} {{ currencyTitle() }}</td>
                                    @endif
                                    <td>{{ trans('front::messages.currency.prefix') }}{{ number_format($item->quantity * ($priceCheck ?? $item->realPrice())) }} {{ currencyTitle() }}</td>
                                    <td>{{ $item->discount ? $item->discount . '%' : 0 }}</td>

                                    <td>{{ trans('front::messages.currency.prefix') }}{{ number_format(($priceAllCheck ?? $item->price) * $item->quantity) }} {{ currencyTitle() }}</td>
                                    <td>
                                        @if ($item->product && $item->product->isDownload() && $item->get_price && $item->get_price->isDownloadable())
                                            <a href="{{ $item->get_price->downloadLink() }}"
                                               class="btn btn-success mb-2">{{ trans('front::messages.profile.download') }}</a>
                                        @endif

                                        @if($item->product)
                                            <a href="{{ route('front.products.show', ['product' => $item->product]) }}"
                                               class="btn btn-info mb-2">{{ trans('front::messages.profile.view') }}</a>

                                        @else
                                            <button class="btn btn-info mb-2"
                                                    disabled>{{ trans('front::messages.profile.view') }}</button>
                                        @endif
                                    ...
                                    <table class="checkout-orders-table">
                                        ...
                                        @foreach($order->transactions()->latest()->get() as $transaction)
                                            <tr>
                                                ...
                                                <td class="price">
                                                    <p>{{ trans('front::messages.currency.prefix') }} {{ number_format($transaction->amount) }} {{ currencyTitle() }}</p>
                                                </td>
                                                <td class="status">
                                                    <p>
                                                        @if($transaction->status)
                                                            <span
                                                                class="text-success">{{ trans('front::messages.profile.successful-payment') }}</span>
                                                        @else
                                                            <span
                                                                class="text-danger">{{ trans('front::messages.profile.unsuccessful-payment') }}</span>
                                                        @endif
                                                    </p>
                                                </td>
                                            </tr>
