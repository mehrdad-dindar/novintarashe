                ...
                <div class="row">
                    ...
                    <div class="col-lg-4 col-md-12 ps-relative">
                        ...
                        @elseif($product->special)
                            ...
                            <ul class="gallery-options special ">
                                @if (auth()->check())
                                    @php
                                            ...
                                            <a class="gallery-item mt-3" href="{{ asset($item->image) }}"
                                               data-fancybox="gallery" data-owl="one{{ $loop->index }}">
                                                <img src="{{ theme_asset('images/600-600.png') }}"
                                                     data-src="{{ asset($item->image) }}" alt="{{ $product->title }}">
                                            </a>
                                        </div>
                                    @endforeach
                                </div>
                        ...
                        @else
                            <div class="product-gallery mt-3">
                                <div class="product-carousel owl-carousel">
                                    <div class="item">
                                        <a class="gallery-item mt-3" href="{{ asset($product->image) }}"
                                           data-fancybox="gallery" data-owl="one">
                                            <img src="{{ theme_asset('images/600-600.png') }}"
                                                 data-src="{{ asset($product->image) }}" alt="{{ $product->title }}">
                                        </a>
                                    </div>
                                </div>
                                <hr class="border-product" />
                                <ul class="product-thumbnails product-carousel owl-carousel carousel-products d-flex justify-content-center">
                                    <li class="active">
                                        <a href="#one" class="owl-thumbnail" data-slide="">
                                            <img src="{{ theme_asset('images/600-600.png') }}"
                                                 data-src="{{ asset($product->image) }}" alt="{{ $product->title }}">
                                        </a>
                                    </li>
...
@push('scripts')
    ...
    <script>
        ...
        function eachPrice(){
            $('td.td-price').each(function(index, data) {
                let amount = parseInt($('#cart-quantity').val(), 10); // تبدیل مقدار به عدد
                let min = $(data).data('min');
                let max = $(data).data('max');
                let price = $(data).data('price');
                let defaultprice = $(data).data('defaultprice');

                if (amount >= min && amount <= max) {
                    $('span.price').text(price)
                    return false;
                }else{
                    $('span.price').text(defaultprice)
                }
