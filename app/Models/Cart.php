...
class Cart extends Model
{
    ...
    public function getPriceAttribute()
    {
        ...
        foreach ($this->products as $product) {
            $priceProduct = $product->prices()->find($product->pivot->price_id) ;
            if($priceProduct){
                $priceProduct = $product->prices()->find($product->pivot->price_id) ;
                $discountMajor = $priceProduct->discountMajors->where('min', '<=', $product->pivot->quantity)->where('max', '>=', $product->pivot->quantity)->first();

                if (isset($discountMajor) && $discountMajor != null) {
                    $price += ($discountMajor->discount * $product->pivot->quantity);
    ...
    public function discountPrice()
    {
        ...
        foreach ($this->products as $product) {
            ...
            if($product_price){
                $discountMajor = $product_price->discountMajors->where('min', '<=', $product->pivot->quantity)->where('max', '>=', $product->pivot->quantity)->first();

//            $price += ($product_price->discountPrice() * $product->pivot->quantity);

                $price += $product_price->finalPrice( $product_price->discountPrice() ,  $product->pivot->quantity , $product->pivot->price_id) ;
            }
        }
    ...
    {
        $priceProducts = $this->discountPrice();
        $price  = $priceProducts + $this->gatewayChecks($priceProducts) + $this->shippingCostAmount();
    ...
    public function gatewayChecks($priceProducts)
    {
        return ( $priceProducts * $this->gateway?->config('percent') /100 );
    }
    ...
    public function shippingCost()
    {
        ...
        if ($this->city_id) {
            ...
            return currencyTitle() . number_format($amount) . currencyTitle();
        }
...
//            if (isset($discountMajor) && $discountMajor != null){
//                $priceDiscount = $product_price->tomanPrice() - (($product_price->tomanPrice() * $discountMajor->discount)/100) ;
//
//                $discount += ($product_price->tomanPrice() * $product->pivot->quantity) - ($priceDiscount * $product->pivot->quantity);
//
////                dd( $product_price->tomanPrice());
//
//            }
            }
    ...
    public function canUseDiscount($code = null)
    {
        ...
        if ($discount->least_price !== null && $this->price < $discount->least_price) {
            return [
                'status'  => false,
                'message' => "حداقل قیمت سفارش برای این کد تخفیف " . number_format($discount->least_price) . " ".currencyTitle()." می باشد."
            ];
        }
