...
class Price extends Model
{
    ...
    public function discountMajors()
    {
        return $this->hasMany(DiscountMajor::class, 'price_id');
    }
    ...
    public function createChange($new_price, $new_discount, $name = null, $new_stock = null, $old_price = null, $old_discount = null, $old_stock = null)
    {
    ...
    {
        return get_discount_price($this->price, $this->discount, $this->product, $this->accounting, $this->discount_price);
    }


    public function finalPrice($price, $amount, $price_id)
    {
        $p = Price::find($price_id);


        $result = $p->discountMajors->where('min', '<=', $amount)
            ->where('max', '>=', $amount)->first();

        if ($result != null) {
            $priceAmount = ( $result->discount * $amount);
