...
class Category extends Model
{
    ...
    {
        $parents = collect([]);
        $visitedCategories = []; // آرایه‌ای برای ذخیره دسته بندی‌هایی که قبلاً بررسی شده‌اند

        $parent = $this->parent;

        while (!is_null($parent) && !in_array($parent->id, $visitedCategories)) {
            $parents->push($parent);
            $visitedCategories[] = $parent->id;
            $parent = $parent->parent;
        }
