...
class SettingController extends Controller
{
    ...
    public function updateGateways(Request $request)
    {
        ...
        if ($request->gateways) {
            foreach ($request->gateways as $id => $request_gateway) {
                ...
                if (isset($request_gateway['configs'])) {
    ...
    public function GetAllProductAccounting()
    {
        // if (!checkApiAccounting()){
        //     return response('error');
        // }

        $lastTime = option('last_time_insertFirstTime_product');

        $lastTimeTimestamp = strtotime($lastTime);
        $currentTimestamp = time();
        $timeDifference = $currentTimestamp - $lastTimeTimestamp;
        ...
        if (true) {
            ...
            return response('error-time');
        }

    }
    public function GetNewProductAccounting()
    {
        // if (!checkApiAccounting()){
        //     return response('error');
        // }
    ...
    public function apply_currency_products()
    {
        ApplyCurrencyProducts::dispatch()->onQueue('apply_currency_to_all_products');
        return response('success');
    }
