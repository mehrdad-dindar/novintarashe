<script id="product-template" type="template/html">
    <div class="row align-items-center order-single-product">
        ...
        <div class="col-md-6">
            ...
            <% } %>
            <strong class="text-success"><span class="sale-price"><%= number_format(product.price) %></span> {{currencyTitle()}}</strong>

            <del class="text-danger regular-price-container <% if (!product.regular_price) { %> d-none <% } %>"><span class="regular-price"><%= number_format(product.regular_price) %></span>
                {{currencyTitle()}}</del>
