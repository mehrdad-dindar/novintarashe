        ...
        <div class="dropdown-menu cart dropdown-menu-sm dropdown-menu-left">
                    ...
                    <a href="{{ route('front.products.show', ['product' => $product]) }}" class="dropdown-item">
                        ...
                        <div class="mr-3 cart-title">
                            {{ $product->title }}
                                ...
                                <div class="pt-1">{{ trans('front::messages.currency.prefix') }}{{ number_format($cart_product_price->discountPrice() * $product->pivot->quantity) }} {{ currencyTitle() }}</div>
                                @if($cart_product_price->discount)
                                    <del class="text-danger">{{ trans('front::messages.currency.prefix') }}{{ number_format($cart_product_price->tomanPrice() * $product->pivot->quantity) }} {{ currencyTitle() }}</del>
                                @endif
            ...
            <div class="dropdown-footer text-center">
                <div class="dt-sl mb-3">
                    ...
                    <span class="float-left">{{ trans('front::messages.currency.prefix') }}{{ number_format($cart->discountPrice()) }} {{ currencyTitle() }}</span>
                </div>
